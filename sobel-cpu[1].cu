...
__global__ void kernel(char *dev_pixels, char *dev_edgeDetection, int imgWidth, int imgHeight)
{
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  if ((x > 0) && (x < imgWidth-1) && (y > 0) && (y < imgHeight-1))
  {
     int answer = sobel(x, y, imgWidth, dev_pixels);
     dev_edgeDetection[y*imgWidth+x] = (char) answer;
  }
}
int main()
{
    ...
    printf("About to use malloc for second time");
    edgeDetection = (char *) malloc(sizeof(char)*imgWidth*imgHeight);
    ...
    for (int i = 0; i < imgHeight; i++)
     ...
     char *dev_pixels;
     char *dev_edgeDetection;
     cudaMalloc((void**) &dev_pixels, sizeof(char)*imgWidth*imgHeight);
     cudaMalloc((void**) &dev_edgeDetection, sizeof(char)*imgWidth*imgHeight);
     printf("Calling kernel function...");

      cudaMemcpy(dev_pixels, pixels, sizeof(char)*imgWidth*imgHeight, cudaMemcpyHostToDevice);
      dim3 blocks(imgWidth/16,imgHeight/16);
      dim3 threads(16,16);
      kernel <<<blocks, threads>>>(dev_pixels, dev_edgeDetection, imgWidth, imgHeight);
      printf("Finished calling kernel function");
      cudaMemcpy(edgeDetection, dev_edgeDetection, sizeof(char)*imgWidth*imgHeight, cudaMemcpyDeviceToHost);
    ...
    for (int i = 1; i < imgWidth-1; i++)
    {
      ...
      {
	int sVal = edgeDetection[j*imgWidth+i];
	aPixel.rgbRed = sVal;
